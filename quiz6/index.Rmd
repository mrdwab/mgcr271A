...
For 1978, we would have Year = 1978 and Manufacturer = 0 (Rawlings). Plugging in the values into the regression equation, we would get $H = -27.9118 + 0.014977 \times 1978$, which gives us $H = `r -27.9118 + 0.014977*1978`$.
...
The $R^2$ can be computed as $SSR/SST$. Using the values given, we get $R^2 = \frac{0.744}{0.744 + 2.539}$, which gives us `r 0.744/(0.744 + 2.539)`
...
```{r echo = F}
ts = qt(0.05, df = 41 - 3, lower.tail = FALSE)
se = 0.00650
```
...
The margin of error for a 90% confidence interval for $\beta_1$ can be calculated as $t^{*} \times se$. The critical t-statistic can be found using $df = 41 - 3$ as `r ts`. Hence, the margin of error is given by `r ts*se`.
...
The t-statistic corresponding to the hypothesis can be computed as $\frac{–0.108}{0.116}$ which equals `r -0.108/0.116`. Consulting the t-distribution table, we find that the p-value will be > 5%.
...
The F-statistic is calculated to be 0.0045 and if follows an F-distribution with 2 and 33 degrees of freedom. The p-value can be computed to be `r pf(0.0045, 2, 33, lower.tail = F)`. See next slide for visualizing the p-value.
...
```{r echo = F}
mu0 = 115; sigma = 25; n = 25; xbar = 116.2
zval = (xbar - mu0)/(sigma/sqrt(n))
...
We can compute the test score as $z = z=\frac{\bar{x}-\mu_{0}}{\sigma/\sqrt{n}}$. Pluggin in values, we get $z = `r zval`$. Using the normal distribution table, we can find $p = `r pval`$. 
...
```{r echo = F}
mu0 = 1.5; sigma = 0.1; n = 100;
alpha = 0.05; zs = qnorm(alpha, lower.tail = FALSE)
xbar = mu0 + zs * sigma/sqrt(n)
mua = 1.6
zs2 = (xbar - mua)/(sigma/sqrt(n))
```
...
Given $\alpha = 5%$, the null hypothesis would be rejected if $z > `r zs`$. This translates to rejecting $H_0$ if we get a sample mean $\bar{x} > `r mu0` + `r zs` \times `r sigma`/\sqrt{`r n`}$,  or $\bar{x} > `r xbar`$. If the alternate were true, then $\mu_a = 1.6$. So the null would be rejected when the z-score exceeds $\frac{\bar{x}-\mu_{a}}{\sigma/\sqrt{n}}$. Plugging in values, we get `r zs2`. Power of the test is the area corresponding to the rejection region under the alternate hypothesis, which clearly will be close to 100%. See next slide for a visualization.
...
A 90% confidence interval can be calculated as $\left(\bar{x}_{1}-\bar{x}_{2}\right)\pm t_{0.005}^{*}\times\sqrt{\frac{s_{1}^{2}}{n_{1}}+\frac{s_{2}^{2}}{n_{2}}}$ . Plugging in the values, we get `r CIans`.
...
The two sample t-statisic can be calculated as $\frac{\bar{x}_{1}-\bar{x}_{2}}{\sqrt{\frac{s_{1}^{2}}{n_{1}}+\frac{s_{2}^{2}}{n_{2}}}}$. Plugging in the correct values, we get $t = `r tval`$.
...
```{r echo = F}
n = 1200; x = 640; phat = x/n;
p0 = 0.5
zval = (phat - p0)/sqrt(p0*(1 - p0)/n)
...
The test statistic for the hypothesis can be computed as $(\hat{p}-p_{0})/\sqrt{\frac{p_{0}(1-p_{0})}{n}}$. Plugging in $\hat{p} = `r phat`$, $p_0 = `r p0`$, we get $z = `r zval`$. From the standard normal table, we can compute the p-value to be `r pval`.
...
```{r echo = F}
C = 0.99; n = 200; x = 80; phat = x/n;
zs005 = qnorm((1 - C)/2, lower.tail = FALSE)
moe = zs005 * sqrt(phat * (1 - phat)/n)
```
...
The margin of error for a 99% confidence interval can be calculated as $z_{0.005}^{*}\times\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}$, where $z_{0.005}^{*} = `r zs005`$ and $\hat{p} = `r phat`$. Plugging in these values we get $m = `r moe`$ .
...
```{r echo = F}
x = 6; n = 150; phat = x/n; p0 = 0.10
zval = (phat - p0)/sqrt(p0*(1 - p0)/n)
pval = pnorm(abs(zval), lower.tail = F)
```
...
```{r echo = F}
x1 = 20; n1 = 100; p1 = x1/n1
x2 = 10; n2 = 100; p2 = x2/n2
se = sqrt(p1*(1-p1)/n1 + p2*(1 - p2)/n2)
zs = qnorm(0.05, lower.tail = F)
moe = zs * se
```
...
```{r echo = F}
x1 = 10; n1 = 25; p1 = x1/n1
x2 = 5; n2 = 20; p2 = x2/n2
se = sqrt(p1*(1-p1)/n1 + p2*(1 - p2)/n2)
zs = qnorm(0.025, lower.tail = F)
moe = zs * se
```
...
The standard error of the sampling distribution can be computed as $\sqrt{\frac{\hat{p}_{1}(1-\hat{p}_{1})}{n_{1}}+\frac{\hat{p}_{2}(1-\hat{p}_{2})}{n_{2}}}$. Plugging the correct values, we get $se = `r se`$. Hence the margin of error for a 95% confidence interval equals $m = z^{*} \times se$, which gives us $m = `r zs` \times `r se`$, which equals `r moe`.
...
The proportion of people who consider themselves Independent is given by (18 + 31 + 19 + 48 + 9)/720, which gives us `r (18 + 31 + 19 + 48 + 9)/720`.
...
The proportion of Democrats who believe Health Care is the main concern can be computed as 88/(45 + 102 + 50 + 88 + 17), which gives us `r 88/(45 + 102 + 50 + 88 + 17)`
