...
A 90% confidence interval for $\mu$ can be calculated as $\bar{x} \pm z^{*} \times \sigma/\sqrt{n}$. Pluggin in values, we get $25 \pm `r 1.645*6/sqrt(9)`$.
...
We can compute from the data $\bar{x} = `r mu`$. Hence, we can compute the 95% confidence interval as $`r mu` \pm `r 1.96*10/sqrt(n)`$.
...
```{r echo = F, comment = NA}
x3b = 100*c(0.33, 0.39, 0.32, 0.36)
mu3b = mean(x3b)
n3b = length(x3b)
z3b = qnorm(0.995, lower.tail = T)
cat(sprintf('%s%%', x3b))
```
...
We can compute from the data $\bar{x} = `r mu3b`$. Hence, we can compute the 95% confidence interval as $`r mu3b` \% \pm `r z3b*9/sqrt(n3b)` \%$.
...
The margin of error is given by $m = z^{*}\times \sigma/\sqrt{n}$. Hence, the sample size required for a given margin of error can be computed as $\left(\frac{z^{*}\sigma}{m}\right)^{2}$. Substituting $z^{*} = `r zstar`$, $m = 1\%$ and $\sigma = 9\%$, we get $n = `r n`$. We round up the answer to get $n = `r ceiling(n)`$.
...
We can compute compute the 92% confidence interval as $5.82 \pm `r qnorm(0.96)*1.11/sqrt(49)`$
...
We can compute the test score as $z = z=\frac{\bar{x}-\mu_{0}}{\sigma/\sqrt{n}}$. Pluggin in values, we get $z = `r zval`$. Using the normal distribution table, we can find $p = `r pval`$. See next slide for visualizing the p-value.
...
```{r echo = F}
mu0 = 10; sigma = 0.4; xbar = 9.8; n = 100
zval = (xbar - mu0)/(sigma/sqrt(n));
pval = round(2*pnorm(abs(zval), lower.tail = F), 4)
```
...
We can compute the test score as $z = z=\frac{\bar{x}-\mu_{0}}{\sigma/\sqrt{n}}$. Plugging in values, we get $z = `r zval`$. Using the normal distribution table, we can find $p \sim `r pval`$. See next slide for visualizing the p-value.

--- &vcenter

<iframe src='http://www.statdistributions.com/normal?z=`r zval`'></iframe>
...
```{r echo = F}
xbar = 1.53; sigma = 0.1; mu0 = 1.5; n = 100
zval = (xbar - mu0)/(sigma/sqrt(n))
...
We can compute the test score as $z = z=\frac{\bar{x}-\mu_{0}}{\sigma/\sqrt{n}}$. Plugging in values, we get $z = `r zval`$. Using the normal distribution table, we can find $p \sim `r pval`$. Since $p \le \alpha$, we reject the null hypothesis $H_0$.
...
We can compute the test score as $z = z=\frac{\bar{x}-\mu_{0}}{\sigma/\sqrt{n}}$. Plugging in values, we get $z = `r zval`$. Using the normal distribution table, we can find $p = `r pval`$. Since $p \gt \alpha$, we can NOT reject the null hypothesis $H_0$. See next slide for a visualization.
...
```{r echo = F}
pval = 0.022
zval = qnorm(pval, lower.tail = F)
zs1 = qnorm(0.05, lower.tail = F)
zs2 = qnorm(0.005, lower.tail = F)
```
...
Based on the p-value, we get that the sample statistic $\bar{x}$ is $z = `r zval`$ standard errors away from 1 (the mean according the the null hypothesis). A 95% confidence interval for $\mu$ would consist of values lying within `r zs1` standard errors from $\bar{x}$. Since 1 lies `r zval` standard errors from $\bar{x}$, it won't lie in the 95% confidence interval. However, a 99% confidence interval for $\mu$ would consist of values lying within `r zs2` standard errors from $\bar{x}$ and since 1 lies `r zval` standard errors from $\bar{x}$, it will lie in the 99% confidence interval. The question of whether ZERO would lie within the 95%/99% intervals will depend on the actual values of $n$. 
